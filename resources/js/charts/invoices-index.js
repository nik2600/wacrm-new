export default function init() {
    ...
    const invModal = document.getElementById('manual-invoice-modal');
    if (invModal) {
        const open  = () => { invModal.style.display = 'flex'; invModal.querySelector('[name="workspace_id"]')?.focus(); };
        const close = () => { invModal.style.display = 'none'; };
        document.querySelectorAll('[data-manual-invoice-open]').forEach((b) => b.addEventListener('click', open));
        invModal.querySelectorAll('[data-manual-invoice-close]').forEach((b) => b.addEventListener('click', close));
